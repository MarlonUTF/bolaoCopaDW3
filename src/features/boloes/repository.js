...
class BolaoRepository {
  ...
  async criar(nome, descricao, criador_perfil_id) {
    const query = `
  ...
  async buscarParticipante(bolao_id, perfil_id) {
    const query = 'SELECT * FROM participantes_bolao WHERE bolao_id = $1 AND perfil_id = $2'
    const resultado = await pool.query(query, [bolao_id, perfil_id])
    return resultado.rows[0] || null
  }
